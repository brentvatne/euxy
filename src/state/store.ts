...
export interface AppState {
  ...
  /** Flip a lane's randomization lock (locked lanes skip mutate/dice rolls). */
...
export const useStore = create<AppState>((set, get) => {
  ...
  return {
    ...
    // Lock gates the PATTERN-WIDE randomizers only (mutate + dice roll). The
    // Lane Editor's own Randomize stays live — you asked for THAT lane by
    // name, so the lock has nothing to protect you from.
