 ...
 * bossa). Drum lanes all target channel 0 — the OP-XY's track 1 drum kit —
 * using the factory slot notes (see core/opxy.ts); melodic lanes sit on
 * instrument tracks.
 *
...
export function presetPatterns(): Pattern[] {
  return [
    ...
    pattern('preset_cinquillo', 'Cinquillo', 110, [
      ...
      { name: 'Bass', length: 8, genA: { pulses: 3, rotation: 2 }, note: 48, channel: 2, velocity: 88, gateMs: 110 },
    ]),
  ];
}
